...
class PeopleCountProblem(VoiceoverScene):
    ...
    def show_intro(self):
        ...
        problem_text = MarkupText(
            "一班有<span fgcolor='#FFFF00'>56</span>人，<br>"
            "女生走了<span fgcolor='#EB5757'>1/3</span>，男生走了<span fgcolor='#2F80ED'>1/4</span>。<br>"
    ...
    def setup_variables(self):
        ...
        setup_text = MathTex(r"\text{设男生 } x \text{ 人，女生 } y \text{ 人}", color=WHITE)
    ...
    def show_fraction_bars(self):
        ...
        with self.voiceover(text="男生走了四分之一，剩下就是四分之三x"):
            self.play(
                boy_removed.animate.set_fill(BLACK, opacity=0.8).set_stroke(style=DASHED_LINES),
                FadeIn(boy_remain_brace), Write(boy_remain_text)
            )

        with self.voiceover(text="女生走了三分之一，剩下就是三分之二y"):
            self.play(
                girl_removed.animate.set_fill(BLACK, opacity=0.8).set_stroke(style=DASHED_LINES),
...
print(sol)
        """
        code_text = Code(code=code_str, tab_width=4, background="window", language="python", font="Monospace", insert_line_no=False, style="monokai")
        code_text.code.scale(0.8)
