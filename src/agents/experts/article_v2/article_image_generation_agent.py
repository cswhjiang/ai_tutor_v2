...
class ArticleImageGenerationAgent(BaseAgent):
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        draft_results = ctx.session.state.get('article_generation/draft_results', {})
        ...
        # logger.info(draft_results)
        if 'reference_image_name' not in current_parameters and 'article_image_to_generate' not in draft_results: # NOTE： 不需要有图呢？
            error_text = f"提供给{self.name}的参数缺失，必须包含： reference_image_name 或者 article_image_to_generate"
        ...
        logger.info(img_obj)
        ag_image_to_generate = draft_results.get('article_image_to_generate', [])
        logger.info(f'{len(ag_image_to_generate)} images to generate')
        logger.info(ag_image_to_generate)
        ...
        as_set = set()
        for i, img_info in enumerate(ag_image_to_generate):
            aspect_ratio = "1:1"
            resolution = "1K"
            # description = ''
        ...
        if parallel_mode: # 宽高比相同，因此一次全部生成
            if img_obj is None or len(img_obj) == 0: # 无参考图像
                pre_prompt_all = f"你需要为一个文章生成配图，如下是这些图像的提示词。尤其需要注意的是图像上的文字需要符合prompt的要求，不要出现类似比例、参数的不该出现的信息。你需要生成{len(img_info_to_generate)}个单独的图像。\n"
            else:
                pre_prompt_all = f"你的任务是参考一个或多个指定的图像来为一个文章生成配图。尤其需要注意的是图像上的文字需要符合prompt的要求，不要出现类似比例、参数的不该出现的信息。如下是这些需要生成的插图的提示词。你需要生成{len(img_info_to_generate)}个单独的图像。\n"

            for i, info in enumerate(img_info_to_generate):
                if i == 0:
                    prompt = pre_prompt_all + f'这是第{i}个图像的提示词：' + info[0] + '\n'
                else:
                    prompt =  f'这是第{i}个图像的提示词：' + info[0] + '保持与前一个生成图像中的人物、服装、场景风格一致。' '\n'
                prompt_list.append(prompt)

            logger.info('一次性调用nano banana')
            logger.info(prompt_list)
            logger.info(aspect_ratio)
            results = await nano_banana_image_generation_tool(img_obj, prompt_list, aspect_ratio=aspect_ratio, resolution="2K")
            ...
            if results['status'] == 'success':
                ...
                for img_index, img_bytes in enumerate(results["image_bytes_list"]):
                    ...
                    if file_name_placeholder:
                        ...
                        artifact_name = f"step{ctx.session.state.get('step') + 1}_article_image_generation_output{img_index}_{file_name_placeholder_remove_dot}.png"
                    else:
                        artifact_name = f"step{ctx.session.state.get('step') + 1}_article_image_generation_output{img_index}.png"
        ...
        if not parallel_mode: # 宽高比不同，一次生成一张图像
            ...
            for i, img_info in enumerate(ag_image_to_generate):
                ...
                if results['status'] == 'error':
                    ...
                    if file_name_placeholder:
                        file_name_placeholder_remove_dot = file_name_placeholder.replace('.', '_') ## abc.png -> abc_png
                        artifact_name = f"step{ctx.session.state.get('step') + 1}_article_image_generation_output{i}_{file_name_placeholder_remove_dot}.png"
                    else:
                        artifact_name = f"step{ctx.session.state.get('step') + 1}_article_image_generation_output{i}.png"
        ...
        if len(output_artifacts) == 0 and len(ag_image_to_generate) > 0:
        ...
        yield self.format_event(message, {"article_image_generation_results": current_output})
        return
