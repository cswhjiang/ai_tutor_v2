...
# 输出要求
 ...
 - 每个页面上都需要注意字体、对齐、装饰。
 - 可以利用ReactDOM的jsx代码来表示各个页面素材的位置，结果放入`ppt_jsx`字段
...
| 字段名称 | 类型 | 描述 |
| :--- | :--- | :--- |
| `ppt_draft` | json | 包含完整的json格式的PPT设计，每个页面分开，内部的插图以placeholder的形式包含在文章里面，页面内需要用来画图的数据也需要单独说明。|
| `ppt_jsx` | 字符串 |  用ReactDOM表示的 PPT的设计稿的完整内容，内部的插图以placeholder的形式包含，页面内需要用来画图的数据也需要单独说明。可以认为是ppt_draft 的代码化表示。 |
| `ppt_image_to_generate` | 数组 | 存放需要向图像生成智能体请求的**图像信息清单**。如果不需要生成任何图像，则为 `[]`。 |
