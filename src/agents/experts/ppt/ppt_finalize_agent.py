...
class PPTFinalizeAgent(BaseAgent):
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        if len(text_list) == 0:
            ...
            message = "PPTFinalizeAgent 已完成设计稿"
            message_for_user = "生成回复成功"
...
最后由其他智能体参考你的设计用gemini-3生成网页代码的方式来输出整个PPT。
...
# 尺寸、版式、风格
 - PPT的每个页面的宽高比固定用 16:9，也就是 33.867 cm x 19.05 cm，144 DPI，像素尺寸为 1920 x 1080，适合高清投影 和日常使用。
...
| 字段名称 | 类型 | 描述 |
| :--- | :--- | :--- |
| `ppt_final` | json | 包含完整的json格式的PPT设计，每个页面分开，插图以生成的图像文件名的形式包含在设计稿里面。占位符必须被对应的文件名取代。，页面内需要用来画图的数据也需要单独说明。|
| `ppt_jsx` | 字符串 | 用ReactDOM表示的 PPT的设计稿的完整内容，插图以生成的图像文件名的形式包含在代码里面。|
| `ppt_image_name_list` | list，元素为字符串 | 包含 `ppt_final` 中需要全部的图像的真实文件名，需要与`ppt_final`文本中的文件名一致，一定含有 'ppt_image_generation_output' 字符串。|
