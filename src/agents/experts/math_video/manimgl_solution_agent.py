...
MANIMGL_SOLUTION_KEY = "math_video/manimgl_solution"


async def manimgl_solution_before_model_callback(
    callback_context: CallbackContext,
    llm_request: LlmRequest,
):
    """Build the model request for ManimGL solution generation."""
...
class ManimGLSolutionAgent(BaseAgent):
    """Generate the math solution used by the ManimGL route."""
    ...
    def __init__(self, name: str, description: str = "", llm_model: str = ""):
        ...
        llm = LlmAgent(
            ...
            **model_kwargs,
            ...
            instruction=MANIMGL_SOLUTION_INSTRUCTION.format(TIME_STR=time_str),
            before_model_callback=manimgl_solution_before_model_callback,
            output_key=MANIMGL_SOLUTION_KEY,
        )
        super().__init__(name=name, description=description, llm=llm)

    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        """Run the ManimGL solution agent and publish a step status event."""
        ...
        with timing_stage(
            ...
            **timing_context,
            ...
            with timing_stage(
                ...
                f"{self.name}.llm",
                **timing_context,
                metadata={"output_key": MANIMGL_SOLUTION_KEY},
            ):
